...
export const createCost = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!costHead_id || !amount) return res.status(400).json({ success: false, code: 400, message: "costHead_id and amount required!!!" });

        const isExistsHead = await CostCategory.findByPk(Number(costHead_id));
        if (!isExistsHead) return res.status(404).json({ success: false, code: 404, message: `Cost Head not found!!!` });

        if (costSubHead_id) {
            const isExists = await CostCategory.findByPk(Number(costSubHead_id));
            if (!isExists) return res.status(404).json({ success: false, code: 404, message: `Cost Sub Head not found!!!` });
        };
        ...
        if (!costCenter) return res.status(500).json({ success: false, code: 500, message: "Insertion failed!!!" });
...
export const updateCost = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!costCenter) return res.status(404).json({ success: false, code: 404, message: "Cost record not found!!!" });
